#!/usr/bin/env python3
"""Race Top-10 C0 evaluation R03-R12 with persisted artifacts.

The 2026-08-02 breakthrough numbers (grid-only baseline vs the frozen
optimized model) were printed by ad-hoc scripts but never persisted.
This runner produces the durable artifact.
...
def main() -> None:
    ...
    out = args.output_dir or (
        root / "outputs/experiments/race" / f"c0_race_r03_r12_{commit[:8]}"
    )
    ...
    for name, fn in candidates.items():
        ...
        results[name] = {
            "per_round": per_round,
            "mean_c0_r03_r12": round(mean(scored), 6) if scored else None,
            "n_rounds": len(scored),
        }
        print(f"{name:<24} mean {results[name]['mean_c0_r03_r12']}  rounds {len(scored)}")
    ...
    payload = {
        ...
        "cutoff_semantics": {
            ...
            "labels": "race_results finish order from actuals.json (R12 merged 2026-09-05)",
        },
        "run_purpose": "persisted_race_c0_full_coverage_supersedes_print_only_20260802_runs",
