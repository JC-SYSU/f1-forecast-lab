...
def main() -> None:
    ...
    for entry in candidates:
        ...
        r1014 = [v for k, v in scores.items() if k >= 10]
        ...
        summary_rows.append({
            ...
            "variant": sp["variant"],
            "n_rounds": len(allv),
            "mean_c0_r03_r14": round(sum(allv) / len(allv), 6) if allv else None,
            "mean_c0_r03_r09": round(sum(r39) / len(r39), 6) if r39 else None,
            "mean_c0_r10_r14": round(sum(r1014) / len(r1014), 6) if r1014 else None,
            "per_round": {str(k): v for k, v in scores.items()},
        })
    summary_rows.sort(key=lambda r: -(r["mean_c0_r03_r14"] or -1))

    payload = {
        "schema_version": "qualifying.c0_roster.v2_full_r03_r14",
        "season": 2026,
        ...
        "cutoff_semantics": {
            "merge_note": (
                "Full recompute R03-R14 under the seat-rotation policy "
                "(manual registry overrides, split reliability, slot entry "
                "filter). All rounds are after-the-fact evidence; the C0 "
                "scoring contract itself is unchanged."
            ),
            "training": "only rounds strictly before each target round",
            "labels": "see label_provenance per round",
        },
        "run_purpose": "seat_rotation_impl_full_recompute_r03_r14_supersedes_a90bf4ad",
    ...
    lines = ["rank,ordinal,candidate_id,model_id,variant,mean_c0_r03_r14,mean_c0_r03_r09,mean_c0_r10_r14"]
    for i, row in enumerate(summary_rows, 1):
        lines.append(
            f"{i},{row['ordinal']},{row['candidate_id']},{row['model_id']},"
            f"\"{row['variant']}\",{row['mean_c0_r03_r14']},{row['mean_c0_r03_r09']},{row['mean_c0_r10_r14']}"
        )
    ...
    for row in summary_rows[:5]:
        print(
            ...
            f"r03-14 {row['mean_c0_r03_r14']}  (dev {row['mean_c0_r03_r09']} | late {row['mean_c0_r10_r14']})"
        )
