...
The screening was mechanical rather than a judgment call, and it is stated here so the 74-to-45 step can be checked: a record entered the deep-evaluation set if its declared applicable targets included qualifying and its first-round transferability was graded high or medium. Both fields were filled in during the metadata scan, before any full text was read, so the cut could not be influenced by what the papers turned out to say. What the grade meant is worth one line, because it is coarse by design: high and medium together admitted any method with a plausible route to our data, and the feasibility gate of this section is what removed the ones that could not actually run.
...
- **P2 · The lap-time conditional layer.** The precondition is aggregating raw data into session-level features; it outputs predicted lap times and gaps relative to pole, and under the constraint of 3.3 it takes part in the ranking only when calibration improves both at the same time.
