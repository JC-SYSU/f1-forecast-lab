...
The chapters before this one covered methods; from here on, they cover models. Two independent model lines run in this project, the qualifying model and the race model, and they are told here as two separate stories, in two halves of this chapter. They never share parameters, and their ways of taking shape differ completely: the qualifying line spread out a whole group of candidates in one move and let them compete under a single scoring standard, through internal evolution and elimination, to the pair of assemblies that now leads it; the race line set out from a minimal baseline, ran into a technical problem that disabled prediction partway, repaired it, froze, and was finally put through a formula-slot audit in which every sibling variant of its key feature was tried in its place. The incumbent won. Cross-line events (a shared scoring scale, a seat-rotation policy, a practice-signal experiment) appear in both stories where they belong, but each story stands on its own.
...
graph LR
    ...
    subgraph R["Race line"]
        R1["Grid-position baseline"] --> R2["Multi-factor first version<br/>failed paths"] --> R3["Feature analysis<br/>209 variants"] --> R4["Problem diagnosis<br/>standardization fix"] --> R5["Freeze<br/>prospective validation"] --> R6["Formula-slot audit<br/>incumbent confirmed"]
    end
```

## 6.1 The Qualifying Line · Starting Point: The First Version and Two Variants

The factors that go into a qualifying result can be named by anyone who follows the sport, and they arrange themselves into a short list. A driver on a run of strong Saturdays is likely to keep going, which is recent form. The car underneath matters as much as the driver, which is constructor strength. A circuit's own history at the same track says something about who goes well there, which is circuit history. Between the sessions, paddock reporting on setup choices and upgrade feedback moves expectations, which is pre-race evidence. And a driver who has been retiring frequently is less likely to produce a clean lap, which is reliability. One more sits slightly apart: not a factor in the result so much as a property of the track. This is its character (high downforce or street, for instance), and it plausibly changes how much the other factors are worth.

That list is where the qualifying line began. The survey behind the project (chapter 03) had already weighed the broader landscape: forty items of literature and practice records backing the score-based route against five for the lap-time route. The first implementation turned each factor on the list into a number, every term converted to a score between 0 and 1 and the terms then added with fixed weights. That conversion is the whole trick, and its meaning can be stated plainly: the factors are the ones humans name; the parameters are where each factor sits, how much it counts, and what "counts" mathematically means. The rest of this half of the chapter is the story of what happened when that handful of human factors was handed to experiments: which terms survived, which comparison axes collapsed, and how a field of 31 models built on these factors evolved under one scoring standard.

The first version of the qualifying model was simple: compute one total score per driver, line up all 22 drivers of the field by that score, and take the top ten as the prediction. What "predict the top ten" narrows here is the scope of the task: the model still ranks the whole field internally, and only the output and the scoring concern themselves with the top ten. As for the lap times and relative pole gaps promised in 1.3, they are not in this version. They belong to the lap-time layer planned later (Chapter 03), which did not exist yet.
...
These weights came from the first-version implementation route settled on after the survey; they are a set of starting values — the survey report explicitly recommended adjusting them later with forward validation. One of the five terms started with a different kind of footing than the rest: form, constructor strength, circuit fit and reliability are computed from data the pipeline already holds, while pre-race evidence depends on a collection routine that has to be run by hand every week. A term whose input is a process rather than a dataset carries that process's risks with it — where those risks led, and how the term's story ended, is told in 6.10.

"Explainable" was not a slogan but three things that can be checked: take any driver's total score and it can be decomposed back into those five raw numbers; the weights sit in the open from start to finish and can be questioned and adjusted one at a time; and if a term lacks data for a round, that term is dropped and the remaining weights renormalized in proportion, with the gap recorded as a gap rather than a zero score passing for data.
...
We backtested the two variants against the eight rounds then complete (rounds 2 through 9); evaluation at the time still ran on the old metrics, which were retired in 6.3. This comparison could not answer the question we actually wanted to ask: the two variants come from the same idea, so comparing them can only settle which approach is better inside that one idea. To judge whether the idea itself is worth continuing, methods built on other ideas have to enter the comparison as well; otherwise "better or worse" has no reference.
...
On the count itself: round 2 has the least usable history, and the two learning-to-rank candidates need more training data before they can start, so they sit out that round. That is why the 230 in Chapter 05's "29 candidates × 8 rounds, 230 scoring opportunities in all" is 2 short of 29 × 8 = 232.
...
The real change came from a lopsided tuning result: set the "circuit fit" weight of the interaction variant of the explainable score to 0.35, and the overall score is only 0.4263 (single candidates topped out around 0.47 at the time, so this sat mid-to-low) — yet the Top5 hit rate reached 0.800. It was weak in the other position segments and strong in the P4–P5 band. That raised the question whether different candidates could each take charge of the position segment where they are strongest.
...
The two final versions differ only in how the stitching order is handled: the robust version returns the same result under both orders, the peak version holds under only one of them. They represent two orientations, "stability" and "ceiling", so both stayed on the roster for further testing. Two things must be said. The ensemble's components were picked and measured on this same window, and "all seven folds positive" tests stability, which is no substitute for unseen data; and the assembled versions are combinations of components drawn from the 29 candidates, so they are not independent of their source candidates. Keep both points in mind when reading their results.
...
Finally, the state of selection at this point: the qualifying line had not chosen a "single entrant", and all 31 candidates were kept, to be tested further by later races. The two slot ensembles with the highest scores (#30, #31) were the decision priorities of the moment, and their story, a lead that shrank out of window, a mid-season scoring stoppage, and the recompute that settled the current order, continues in 7.3. The complete single-race scores of all candidates in these two races are in Appendix C.
...
The object of the scan was the "grid delta" feature family: for every driver in every race, the difference between grid position and finishing position becomes a historical record of where the driver started, where the driver finished, and whether the amount above (or below) expectation was positive or negative. From it we built statistical variants under different measures: aggregation (mean, hit rate, and so on) × correction method × observation window, **208 variants** in all, plus one reference item (the raw grid position itself), 209 entries in total. The variant axes deserve a second look, because they are human judgment made mechanical. "Mean" takes a driver's recovery record at face value; "hit rate" asks only how often the driver finished ahead of the grid; a trimmed or Winsorized mean discounts the freak results; an exponentially weighted or linearly decaying mean treats recent races as more informative than old ones; a Bayesian-shrunk estimate distrusts small samples; a recent-mean variant simply limits the window. Each is a different answer to the same question, how a careful person would summarize this driver's history, and the scan let the data argue between them. For each entry we computed its correlation with finishing order (Spearman ρ: a coefficient of how alike two orderings are; 1 means identical). The reference item's value was **0.6729**, the predictive power of grid position itself. The full distribution falls into two clusters: most variants sit in negative territory, and a cluster of stronger variants gathers around **0.63**. None of them beats the reference item.
...
Second, a lead. One class of statistic goes by the name **racecraft** (the recovery-rate record) — the share of a driver's historical races in which the actual finishing order beat what the grid position implied. Its overall correlation is unremarkable, but split by position segment its behavior seems concentrated in the lower half of the field (grid P11 and beyond), and close to noise in the front range. We logged it as a lead to be verified: this feature may deserve a place in the "back of the grid", and none in the "top ten".

The lead posed the next design question directly: if recovery acts mainly in the back of the grid, then a correction could be added to the predicted positions of back-of-grid drivers, giving the best recoverers a chance to reach the top ten.
...
**Diagnosis.** We ran correlation checks on 198 samples across R03–R11. The results: recent form (hereafter form) has a Pearson correlation coefficient of **-0.869** against grid position, constructor strength **-0.771**, racecraft **-0.531**, circuit fit -0.314; the pre-race-evidence term was 0 across this whole batch of data — the cause went unidentified at the time, and the question stayed on the to-do list for the rest of the writing period; it has no variance and cannot take part in the computation. High correlation is not itself a bad thing; the problem is direction. Strong teams qualify and start up front — and form, constructor strength, and racecraft all score those same strong teams highly: every term is pushing the same set of drivers toward the same side.
...
Each driver's adjustment was therefore decided almost entirely by that driver's grid position: about -2.8 at pole, about -1.0 at tenth (negative means a forward correction; the numbers are movements of score, not of position). There is a mathematical consequence here. If the adjustment varies monotonically with grid position, with more push for those in front and less for those behind, then the new score is an order-preserving transform of the original grid position, and the ordering cannot change, however hard you push. A debug script confirmed it on R10 and R11: not one of the ten predicted positions changed. We later gave this class of phenomenon a name, "the monotone-transform problem": every correction stacks along the same monotone direction, so no new order can be produced. (On the measure: the 6.6 scan measured the correlation of feature variants with finishing order; here it is the correlation of features with grid position, so the objects differ.)
...
**The weight mix.** The final formula's three-term mix is form 3.0, racecraft 2.0, rear-of-grid interaction 3.0. The provenance has two layers: a grid search on the training window (R03–R09) first, then verification on the test window (R10–R11). Behind these three numbers sit three judgments. Recent form matters more than recovery history (3.0 > 2.0). Constructor strength does not enter the linear main term on its own, because its information is already carried in bulk by form and racecraft, and the training-window search did not pick it as a standalone term. And the rear-of-grid interaction weighs the same as form. The mix needs care in reading: one 3.0 acts on a standardized feature (unit: standard deviations), the other 3.0 acts on c (unit: a strength value from 0 to 1), so the two cannot be compared directly. Actual strength speaks through test-window performance.

In total, **+8.81%** over the baseline (0.5438 → 0.5917).

![The race model's three-stage lift](../assets/charts/race-upgrade.svg)

*Figure: the three-stage lift from grid-position baseline to final model (test-window means).* Several limits apply. The percentages are the customary way to express relative gain, C0 has no natural zero (Chapter 05), and these comparisons make no claim of statistical significance (04.4). The test window holds only two races, and the overfitting risk has not been quantified. And the qualifying line's seven-fold leave-one-out validation was never replicated on the race line, whose protocol at the time was simply a fixed train/test split.
...
R12 was the third window's first round. Facing data it had never seen, the model got the first four positions, names and order, entirely right (it moved Antonelli from the baseline's P3 up to P2, and the actual result was indeed P2), and finished ahead 0.6155 to the same-race baseline's 0.4614 (single-race C0 measure, not directly comparable with the two-race means of 6.5–6.7). The details are on record as they are. Hulkenberg climbed from P13 to P8 and Alonso from P18 to P9, and both finished inside the real top ten, but the push of the rear-of-grid interaction was not enough to carry them into the model's predicted top ten. The model's predicted P7 went to Verstappen, who retired from that race, so the slot was bound to come up empty (drivers who fail to finish do not enter the actual-result comparison set); that is a case of randomness no one could have foreseen. The readings from R13 and R14 follow in Chapter 07.

The third window also produced the season's hardest data problem, and with it the one mechanism change the race line has made since the freeze. Around R12 a driver roster change (the full chain in 7.3) left six qualifying candidates unscoreable and, by R14, had stopped the forward evidence of the two leading ones. The race line paid its own price honestly: under the seat registry's constructor attribution, the recomputed R13 fell from 0.6672 to 0.5864, because the model's predicted top ten gained Lawson at P9, where the actual finish lay outside the top ten. The seat-rotation policy (decision log, entry 12) settled the problem point by point, with constructor-linked parameters following the new team, driver-linked ones travelling with the driver, and a fail-closed seat-event registry filtering the slot assembly, and both lines were recomputed in full over R03–R14 on the new input contract. The freeze itself held: no weight moved, and what changed was exactly what the policy claimed, the inputs' contract where the roster did.
...
## 6.9 The Formula-Slot Audit: The Incumbent Faces Its Family

Freezing settles a model into its formula; it does not settle whether the formula picked the right feature. The racecraft source inside the frozen model came out of the August single-factor scan (6.6) — a choice made by correlation rank, before any formula existed. What that scan never tested is the place the feature actually occupies: the racecraft term of the frozen formula itself. The two views are not the same. A single-factor scan answers "which variant correlates with the result"; a formula-slot ablation answers "which variant is best inside *this* formula, with its standardization, its companions, and its weights" — a variant that ranks fifth in bare correlation can outperform one that ranks second, once the rest of the formula has its say.

So once the seat-rotation recompute had settled the official caliber, the full family went through. The grid-delta registry implements 26 base shapes × 2 rescalings × 4 windows = **208 combinations** (the August scan's statistical family, now parameterized and fully realized); 204 were usable (four columns are empty at small-sample rounds). Each was substituted into the formula's racecraft term one at a time — the frozen model itself untouched, the weights untouched — and rescored over the full R03–R14 window on the official caliber. The audit's zero-patch baseline was verified against the official race artifact round by round to under 1e-6, so every delta in the table is the variant's own effect.

![NSW variant space: the three dimensions and the 208 combinations](../assets/charts/nsw-space-guide.svg)

*Figure: the audit's search space as three axes. N: how each driver's recovery record is summarized (a raw delta; position bands; curve fits; non-parametric smooths; piecewise segments; ten summary statistics of the linear fit). W: how much history counts and how fast it fades (exponential decay, or the last 3/5/7 races). S: what common scale each value goes on (centering or z-score). One small dot is one candidate feature: 26 x 2 x 4 = 208 combinations, 204 of them usable; the incumbent's source (hit rate, exponential window, centering) is marked in red. In the frozen formula the S pair scores identically, so the space holds 104 distinct orderings.*

**The incumbent finished first.** The source in the frozen model (the hit-rate style statistic of 6.6–6.7; its nearest kin in the registry is tagged `N2f6_S0_Wexp`) topped the ranking at **0.5581**. The formula standardizes its racecraft input across the round's field anyway, so the registry's two rescaling variants score identically: the 208 combinations collapse to 104 distinct orderings, and the scan says nothing the formula cannot hear.

Exactly one variant cleared the prospective gate (R10–R14 Δ ≥ +0.010): it gained **+0.018** on the five forward rounds and **lost** 0.028 over the full window. The succession rule had been pre-registered to require both — a full-window gain *and* a prospective lead sustained for at least three forward rounds — precisely so that a favorable five-round slice could not crown itself; the variant fails it, and the reserved form-source stage, which only fires on a qualifying variant, never triggers. The line therefore closes for this cycle: **no sibling variant holds an increment at the formula slot**, the incumbent's August selection is confirmed from the second viewpoint, and the variant registry stays on file as an asset for any future unfreeze (the scan table is indexed in Appendix C).

![Formula-slot audit: 204 variants against the incumbent](../assets/charts/nsw-formula-slot.svg)

*Figure: every variant's score delta against the incumbent, full window (horizontal) against prospective window (vertical). All 204 combinations sit left of the incumbent level; the one variant over the +0.010 prospective gate (red) loses 0.028 on the full window — the region where the pre-registered succession rule is satisfied (right of the incumbent level and above the gate) is empty. S0/S1 rescaling pairs score identically, so the 204 points overlap in pairs (104 distinct orderings).*

With that, the race line's story is complete for the season so far: a baseline that was hard to beat, two failed first attempts, a feature family built out of human recovery heuristics, one diagnosis that unlocked the design, a freeze that put the model into its third window, a seat-rotation recompute that changed the input contract and nothing else, and an audit in which the incumbent saw off its entire family — the current model is, by two independent selections, the best of what was tried.


## 6.10 The Fifth Component: Tested by Reconstruction and Retired (September 22–23)

The qualifying line's September did not end with the practice-signal closure. One member of the founding five still carried an open question: pre-race evidence, the term whose input is a weekly manual process (6.1). It had never scored a round. August's diagnosis (6.7) recorded a term at zero across every sample with no known cause, and on September 20 the cause was found (decision log, entry 15): the anti-leakage gate requires evidence to be retrieved before each round's cutoff, and every historical entry had been back-filled in a single July session, twenty out of twenty correctly refused. The mechanism was sound; the input had never existed. Entry 15's ruling was to fix the process and collect weekly from R15 on.

That ruling was then revised twice, both times by changing the order of work rather than the goal. First, look at the raw material before designing anything: a survey of the official practice-session reports, the intended source, showed the founding schema could not hold what the articles actually contain (one report mixes several kinds of signal), and the signal that had looked most promising was practice pace itself, the line entries 13–14 had just closed on redundancy. What remained worth structuring was threefold: reliability failures, upgrade declarations, and the factual statements teams give after running. Second, build the data layer before any gate code. Twelve rounds had to become comparable although no evidence was ever collected live, and the danger was hindsight: choosing which articles to register after knowing the results is itself contamination, because the selection favors what came true. Three disciplines stood in for the impossible live timestamps. **Mechanical inclusion**: what gets collected per round is fixed in advance as a rule (official and team reports in full), with no human selection. **Existence proof**: part counts come from the FIA's per-round car-presentation submissions, one machine-parsable PDF per round, and each item's availability before the round's cutoff is evidenced by the FIA mirror's own timestamp or an archive snapshot. **Source labeling**: reconstructed and live grades stay distinguishable. Quantification was stepped rather than per-item confidence: every upgrade part ±0.1, every closed-dictionary event ±0.2, summed per driver and clamped, so the annotation judgment reduces to one classification per line. Six standing rules closed the edge cases: pre-cutoff sources only; third parties permanently excluded; R01 out; one upgrade package counted once; rookie rotation treated as noise; signals bound to drivers through the seat registry.

The collection covered R01–R14: 143 archived sources, 57 structured events, machine part counts for all 14 rounds. Then came the three-stage evaluation, judged against criteria frozen before the runs. Correlation, on a 262-row driver-round panel: the only value with any signal was the upgrade-part count, ρ≈0.157 against qualifying order (p≈0.012), and its redundancy check undid it, because the same count correlates −0.160 with that round's constructors' points standing. The parts a team declares are, to first order, its strength wearing a different hat; the term constructor strength already carries it. The events dimension, the original motivation, correlated with nothing. The sum and weighted-combination stages added zero: rank methods make the weights moot. The ablation, run over all 31 candidates with the reconstructed evidence injected, summed #31's paired top-ten hit difference to **+2 against the pre-registered bar of +3**. It was positive in 11 of 12 rounds, but the bar is the bar. Mean C0 moved +0.000013, and 8 of 12 rounds' predictions did not change by a single position.

**The term was retired before its first live round.** The scoring pipeline was cut back to four components, a ~700-line subtraction, verified as an equivalence transform: all 372 candidate-round predictions match the official artifact position for position, and a re-issued verification artifact (`c0_roster_r03_r14_c579e6d0`) reproduces the official scorecard bit for bit. The official caliber `92076830` stands unchanged; nothing needed to be recomputed. What remains in the model are four components that each earned their place on data. On file, the rebuilt archive (143 sources, the structured annotations, the collection pipeline) waits for any future live-collection decision, which would start from it rather than from zero. The ruling is recorded as entry 16, and it confirms the pattern this chapter keeps meeting: a real signal is not a model increment until the C0 scale says so.
