...
## 6.9 Post-publication: The Formula-Slot Ablation (September 20)

This chapter was written on the September 18 basis; this section was added on September 20, when the same question 6.6 left open was re-asked from the other direction.

The 6.6 scan compared 208 grid-delta variants *one factor at a time* — each variant's bare correlation with finishing order — and from that scan the racecraft lead entered the formula (6.7) and was frozen (6.8). What no one had done is put the whole family through the place the feature actually occupies: the racecraft term of the frozen formula itself. The two views are not the same. A single-factor scan answers "which variant correlates with the result"; a formula-slot ablation answers "which variant is best inside *this* formula, with its standardization, its companions, and its weights" — a variant that ranks fifth in bare correlation can outperform one that ranks second, once the rest of the formula has its say.

So on September 20 the full family went through. The grid-delta registry implements 26 base shapes × 2 rescalings × 4 windows = **208 combinations** (the August scan's 208 plus the rescaling axis, one registry, all realized); 204 were usable (four columns are empty at small-sample rounds). Each was substituted into the formula's racecraft term one at a time — the frozen model itself untouched, the weights untouched — and rescored over the full R03–R14 window on the official caliber. The rerun's zero-patch baseline was verified against the official race artifact round by round to under 1e-6, so every delta in the table is the variant's own effect.
