...
The factors that go into a qualifying result can be named by anyone who follows the sport, and they arrange themselves into a short list. A driver on a run of strong Saturdays is likely to keep going — recent form. The car underneath matters as much as the driver — constructor strength. A circuit's own history at the same track says something about who goes well there — circuit history. Between the sessions, paddock reporting on setup choices and upgrade feedback moves expectations — pre-race evidence. And a driver who has been retiring frequently is less likely to produce a clean lap — reliability. One more sits slightly apart: not a factor in the result so much as a property of the track — its character (high downforce or street, for instance) — which plausibly changes how much the other factors are worth.

That list is where the qualifying line began. The survey behind the project (chapter 03) had already weighed the broader landscape — forty items of literature and practice records backing the score-based route against five for the lap-time route — and the first implementation turned each factor on the list into a number: every term converted to a score between 0 and 1, the terms then added with fixed weights. That conversion is the whole trick, and its meaning should be stated plainly: the factors are the ones humans name; the parameters are where each factor sits, how much it counts, and what "counts" mathematically means. The rest of this half of the chapter is the story of what happened when that handful of human factors was handed to experiments — which terms survived, which comparison axes collapsed, and how a field of 31 models built on these factors evolved under one scoring standard.
...
The collection covered R01–R14: 143 archived sources, 57 structured events, machine part counts for all 14 rounds. Then the three-stage evaluation, judged against criteria frozen before the runs. Correlation, on a 262-row driver-round panel: the only value with any signal was the upgrade-part count, ρ≈0.157 against qualifying order (p≈0.012) — and its redundancy check undid it, because the same count correlates −0.160 with that round's constructors' points standing. The parts a team declares are, to first order, its strength wearing a different hat; the term constructor strength already carries it. The events dimension — the original motivation — correlated with nothing. The sum and weighted-combination stages added zero: rank methods make the weights moot. The ablation, run over all 31 candidates with the reconstructed evidence injected: #31's paired top-ten hit difference summed to **+2 against the pre-registered bar of +3** — positive in 11 of 12 rounds, but the bar is the bar — mean C0 moved +0.000013, and 8 of 12 rounds' predictions did not change by a single position.
