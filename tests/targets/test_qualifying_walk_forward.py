...
def test_run_walk_forward_outputs_two_level_metrics() -> None:
...
def test_run_walk_forward_supports_independent_track_profile_weight_profile() -> None:
    payload = run_walk_forward(
        PROJECT_ROOT,
        QualifyingFeatureConfig(track_profile_method="independent"),
    )

    assert payload["config"]["track_profile_method"] == "independent"
    weights = payload["config"]["weights"]
    assert set(weights) == {"form", "constructor", "circuit_fit", "reliability", "track_profile"}
    # keep the track_profile share 0.10 and rescale the rest per DEFAULT to total 0.90
    assert weights["form"] == pytest.approx(0.30 * 0.90 / 0.85)
    assert weights["constructor"] == pytest.approx(0.25 * 0.90 / 0.85)
    assert weights["circuit_fit"] == pytest.approx(0.20 * 0.90 / 0.85)
    assert weights["reliability"] == pytest.approx(0.10 * 0.90 / 0.85)
    assert weights["track_profile"] == pytest.approx(0.10)
