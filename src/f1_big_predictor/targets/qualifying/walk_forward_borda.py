"""Walk-forward harness for the Borda heuristic aggregation model."""
...
def run_walk_forward_borda(
    ...
    for target_round in range(FIRST_WALK_FORWARD_ROUND, LAST_WALK_FORWARD_ROUND + 1):
        ...
        prediction, full_ranking = predict_borda(
            target_round, completed_rounds, weights=w
        )
...
def _ndcg_at_10(predicted: list[str], actual: list[str]) -> float:
    ...
    ideal_dcg = _dcg([_rel(i + 1) for i in range(min(10, len(actual)))])
    return (
        _dcg([_rel(actual_rank.get(did)) for did in predicted[:10]]) / ideal_dcg
        if ideal_dcg > 0
        else 0.0
    )


def _rel(pos: int | None) -> float:
