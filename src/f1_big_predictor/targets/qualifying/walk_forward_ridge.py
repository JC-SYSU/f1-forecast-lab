"""Walk-forward harness for the Ridge multi-factor model."""
...
def run_walk_forward_ridge(
    ...
    """Run Ridge walk-forward over R02–R09 and return results."""
    ...
    for target_round in range(FIRST_WALK_FORWARD_ROUND, LAST_WALK_FORWARD_ROUND + 1):
        ...
        prediction, full_ranking = predict_ridge(
            target_round, completed_rounds, alpha=alpha
        )
        ...
        # Count training samples used for this round
        n_train = sum(
            len(
                [
                    r
        ...
        metrics = {
            ...
            "full_field_rank_mae": round(rank_mae(full_field_ids, actual_ids), 6),
        }
        per_round.append(
            {
                "round": target_round,
...
def _ndcg_at_10(predicted: list[str], actual: list[str]) -> float:
    ...
    ideal = [_relevance(i + 1) for i in range(min(10, len(actual)))]
