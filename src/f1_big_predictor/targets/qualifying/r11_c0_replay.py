"""Isolated R11 qualifying C0 replay using the frozen 31-candidate roster.

The historical roster runner deliberately ends at R09. This module builds an
as-of-R10 overlay from the unchanged R01-R09 actuals plus the FIA-backed R10
qualifying label. R11's FIA result is only used after each candidate produces
its Top 10 for C0 scoring.
"""
...
from .r10_c0_replay import (
    R10_AUDIT_PATH,
    R10_CIRCUIT_HISTORY_FILENAME,
    build_r10_target_event,
    generate_r10_circuit_history_offline,
    load_r10_official_audit,
)


REPLAY_SCHEMA_VERSION = "qualifying.r11_c0_replay.v1"
MANIFEST_SCHEMA_VERSION = "qualifying.r11_c0_replay_manifest.v1"
TARGET_ROUND = 11
SEASON = 2026
EXPECTED_TRAINING_ROUNDS = list(range(1, TARGET_ROUND))
SOURCE_ACTUALS_ROUNDS = list(range(1, 10))
R11_AUDIT_PATH = Path(
    "data/official_labels/"
    "qualifying_v1_r11_official_label_audit.json"
)
R11_CIRCUIT_HISTORY_FILENAME = "r11_hungaroring_circuit_history.json"
R11_RESULTS_FILENAME = "r11_c0_results.json"
R11_MANIFEST_FILENAME = "run_manifest.json"
R11_RANKING_FILENAME = "r11_c0_ranking.csv"
...
def run_r11_c0_replay(
    project_root: Path,
    output_directory: Path,
) -> tuple[dict[str, Any], dict[str, Path]]:
    """Run all frozen candidates for R11 and write reproducible artifacts.
    ...
    r10_audit = load_r10_official_audit(project_root)
    audit = load_r11_official_audit(project_root)
    registry = candidate_registry()
    if len(registry) != 31:
        raise ValueError(f"R11 replay requires the frozen 31-candidate roster, got {len(registry)}")

    base_snapshot = build_input_snapshot(project_root)
    r10_circuit_history_path = output_directory / R10_CIRCUIT_HISTORY_FILENAME
    r10_circuit_history = generate_r10_circuit_history_offline(
        project_root,
        r10_circuit_history_path,
    )
    r11_circuit_history_path = output_directory / R11_CIRCUIT_HISTORY_FILENAME
    r11_circuit_history = generate_r11_circuit_history_offline(
        project_root,
        r11_circuit_history_path,
    )
    ...
    with temporary_r11_overlay(
        ...
        r10_audit=r10_audit,
        audit=audit,
        r10_circuit_history_path=r10_circuit_history_path,
        r11_circuit_history_path=r11_circuit_history_path,
    ) as overlay:
        with extended_walk_forward_round_limit(TARGET_ROUND):
            candidate_results = [
                replay_candidate_r11(spec, overlay.root, audit) for spec in registry
            ]
    ...
    input_snapshot = {
        ...
        "r10_circuit_history": {
            "path": r10_circuit_history_path.name,
            "sha256": _sha256_file(r10_circuit_history_path),
            "matched_historical_event_count": len(
                r10_circuit_history["matched_historical_events"]
            ),
            "skipped_historical_event_count": len(
                r10_circuit_history["skipped_historical_events"]
            ),
        },
        "r11_official_audit": _file_record(project_root, R11_AUDIT_PATH),
        "r11_entry_list": dict(audit["entry_list_source"]),
        "r11_final_qualifying_classification": dict(audit["classification_source"]),
        "r11_structured_crosscheck": dict(audit["structured_crosscheck_source"]),
        "r11_circuit_history": {
            "path": r11_circuit_history_path.name,
            "sha256": _sha256_file(r11_circuit_history_path),
            "matched_historical_event_count": len(
                r11_circuit_history["matched_historical_events"]
            ),
            "skipped_historical_event_count": len(
                r11_circuit_history["skipped_historical_events"]
            ),
        },
    ...
    results = {
        ...
        "run_purpose": "post_event_as_of_r10_single_round_replay_not_production_prediction",
        "cutoff_semantics": {
            "training_rounds": EXPECTED_TRAINING_ROUNDS,
            "target_field": "FIA R11 entry-list universe only",
            "target_result": "FIA final qualifying classification used only after each prediction",
            "r10_historical_label": "FIA R10 final qualifying classification only",
            "r11_openf1_practice_weather_race_data_used_as_model_input": False,
            "r11_agent_evidence_available": False,
    ...
    artifact_paths = write_r11_replay_artifacts(
        project_root=project_root,
...
def load_r11_official_audit(project_root: Path) -> dict[str, Any]:
    """Load and fail closed on the R11 FIA-backed scoring audit."""

    path = project_root / R11_AUDIT_PATH
    payload = _load_json(path)
    validate_r11_official_audit(payload, project_root)
    return payload


def validate_r11_official_audit(payload: dict[str, Any], project_root: Path) -> None:
    """Validate the fixed R11 field, official classification, and source hashes."""

    if payload.get("schema_version") != "qualifying.r11_c0_official_label_audit.v1":
        raise ValueError("unsupported R11 official-label audit schema")
    if payload.get("season") != SEASON or payload.get("target_round") != TARGET_ROUND:
        raise ValueError("R11 official-label audit has the wrong season or round")

    field_size = payload.get("field_size")
    if isinstance(field_size, bool) or not isinstance(field_size, int) or field_size != 22:
        raise ValueError("R11 official-label audit must define the 22-driver field")
    ...
    if not isinstance(entry_field, list) or len(entry_field) != field_size:
        raise ValueError("R11 entry field must contain every one of 22 drivers")
    if not isinstance(official_records, list) or len(official_records) != field_size:
        raise ValueError("R11 official records must cover every one of 22 drivers")

    entry_ids: list[str] = []
    for index, entry in enumerate(entry_field):
        if not isinstance(entry, dict):
            raise ValueError(f"R11 entry_field[{index}] must be an object")
        driver_id = entry.get("driver_id")
        constructor_id = entry.get("constructor_id")
        if not isinstance(driver_id, str) or not driver_id:
            raise ValueError(f"R11 entry_field[{index}] lacks a driver_id")
        if not isinstance(constructor_id, str) or not constructor_id:
            raise ValueError(f"R11 entry_field[{index}] lacks a constructor_id")
        entry_ids.append(driver_id)
    if len(set(entry_ids)) != field_size:
        raise ValueError("R11 entry field contains duplicate driver IDs")

    record_ids: list[str] = []
    positions: list[int] = []
    for index, record in enumerate(official_records):
        if not isinstance(record, dict):
            raise ValueError(f"R11 official_records[{index}] must be an object")
        driver_id = record.get("driver_id")
        position = record.get("position")
        status = record.get("classification_status")
        if not isinstance(driver_id, str) or not driver_id:
            raise ValueError(f"R11 official_records[{index}] lacks a driver_id")
        if isinstance(position, bool) or not isinstance(position, int):
            raise ValueError(f"R11 official_records[{index}] has an invalid position")
        if not isinstance(status, str) or not status:
            raise ValueError(f"R11 official_records[{index}] lacks a classification status")
        record_ids.append(driver_id)
        positions.append(position)
    if len(set(record_ids)) != field_size:
        raise ValueError("R11 official classification contains duplicate driver IDs")
    if positions != list(range(1, field_size + 1)):
        raise ValueError("R11 official classification must be ordered continuously P1-P22")
    if set(entry_ids) != set(record_ids):
        raise ValueError("R11 FIA entry field and final classification driver sets differ")
    ...
    if [driver_id for _, driver_id in raw_rows] != record_ids:
        raise ValueError("R11 Jolpica cross-check does not match the FIA final order")


def generate_r11_circuit_history_offline(
    project_root: Path,
    output_path: Path,
) -> dict[str, Any]:
    """Build the R11 Hungaroring history from cache only, never from network."""

    client = JolpicaClient(
        adapter=_OfflineJolpicaAdapter(),
        archive=RawArchive(project_root),
        archive_week_id="circuit_history_2026_round_11",
    ...
    if len(payload.get("matched_historical_events", [])) != 5:
        raise OfflineCacheMissError(
            "R11 circuit history did not resolve all five archived Hungaroring events"
        )
    if payload.get("skipped_historical_events"):
        raise OfflineCacheMissError("R11 circuit history skipped an event in offline mode")
    if payload.get("evidence_issues"):
        raise OfflineCacheMissError("R11 circuit history has unresolved offline evidence issues")
    for event in payload["matched_historical_events"]:
        for dataset in event.get("datasets", {}).values():
            if dataset.get("status") == "error":
                raise OfflineCacheMissError("R11 circuit history encountered a cache miss")
    if len(payload.get("driver_history_scores", [])) != 22:
        raise ValueError("R11 circuit history must contain 22 current drivers")
    return payload


@contextmanager
def temporary_r11_overlay(
    *,
    project_root: Path,
    r10_audit: dict[str, Any],
    audit: dict[str, Any],
    r10_circuit_history_path: Path,
    r11_circuit_history_path: Path,
) -> Iterator["ReplayOverlay"]:
    """Construct a temporary R01-R10 history plus synthetic R11 target row."""

    with tempfile.TemporaryDirectory(prefix="f1-r11-c0-overlay-") as temporary_directory:
        ...
        if round_numbers != SOURCE_ACTUALS_ROUNDS:
            raise ValueError("R11 replay requires source actuals to contain exactly R01-R09")
        actuals_payload["completed_rounds"] = [
            *rounds,
            build_r10_target_event(r10_audit),
            build_r11_target_event(audit),
        ]
        ...
        _link(
            target_history_directory / "round_11_hungarian_grand_prix.json",
            r11_circuit_history_path,
        )
...
def build_r11_target_event(audit: dict[str, Any]) -> dict[str, Any]:
    """Create the target-round schema row without adding R11 data to training."""
    ...
    return {
        ...
        "qualifying_results": {
            ...
            "field_size": audit["field_size"],
            "official_verification_status": "r11_replay_score_after_prediction",
        },
...
def replay_candidate_r11(
    spec: CandidateSpec,
    overlay_root: Path,
    audit: dict[str, Any],
) -> dict[str, Any]:
    """Run one frozen candidate and score its R11 Top 10 after prediction."""
    ...
    try:
        ...
        if reported_training is not None:
            ...
            if normalized_training != EXPECTED_TRAINING_ROUNDS:
                raise ValueError(
                    "candidate reported target-round training outside R01-R10: "
...
def write_r11_replay_artifacts(
    *,
    project_root: Path,
    output_directory: Path,
    ...
    results_path = output_directory / R11_RESULTS_FILENAME
    results_bytes = _pretty_json(results).encode("utf-8")
    results_path.write_bytes(results_bytes)

    ranking_path = output_directory / R11_RANKING_FILENAME
    ...
    manifest = {
        ...
        "r10_circuit_history_path": R10_CIRCUIT_HISTORY_FILENAME,
        "r10_circuit_history_sha256": results["input_snapshot"]["r10_circuit_history"]["sha256"],
        "r11_circuit_history_path": R11_CIRCUIT_HISTORY_FILENAME,
        "r11_circuit_history_sha256": results["input_snapshot"]["r11_circuit_history"]["sha256"],
    ...
    manifest_path = output_directory / R11_MANIFEST_FILENAME
    manifest_path.write_text(_pretty_json(manifest), encoding="utf-8")
    return {
        "results": results_path,
        "ranking": ranking_path,
        "manifest": manifest_path,
        "r10_circuit_history": output_directory / R10_CIRCUIT_HISTORY_FILENAME,
        "r11_circuit_history": output_directory / R11_CIRCUIT_HISTORY_FILENAME,
    }
...
def _round_result(history: dict[str, Any], round_number: int) -> dict[str, Any]:
    ...
    if not isinstance(matches[0], dict):
        raise ValueError("candidate R11 result must be an object")
    return matches[0]


def _validate_prediction(prediction: list[str], audit: dict[str, Any]) -> None:
    if len(prediction) != 10:
        raise ValueError(f"candidate R11 prediction must contain exactly 10 drivers, got {len(prediction)}")
    if len(set(prediction)) != len(prediction):
        raise ValueError("candidate R11 prediction contains duplicate drivers")
    ...
    if unknown:
        raise ValueError("candidate R11 prediction contains drivers outside the FIA field: " + ", ".join(unknown))
...
def _validate_hashed_source(
    ...
    if not isinstance(source, dict):
        raise ValueError(f"R11 audit {source_key} must be an object")
    if not isinstance(source.get("url"), str) or not source["url"]:
        raise ValueError(f"R11 audit {source_key} lacks a URL")
    expected_sha = source.get("sha256")
    if not isinstance(expected_sha, str) or len(expected_sha) != 64:
        raise ValueError(f"R11 audit {source_key} lacks a SHA-256")
    path = _source_path(project_root, source)
    if not path.is_file():
        raise FileNotFoundError(f"R11 audit {source_key} source is missing: {path}")
    actual_sha = _sha256_file(path)
    if actual_sha != expected_sha:
        raise ValueError(
            f"R11 audit {source_key} SHA-256 mismatch: expected {expected_sha}, got {actual_sha}"
        )
...
def _source_path(project_root: Path, source: dict[str, Any]) -> Path:
    ...
    if not isinstance(raw_path, str) or not raw_path:
        raise ValueError("R11 audit source lacks a relative raw_path")
    relative_path = Path(raw_path)
    if relative_path.is_absolute():
        raise ValueError("R11 audit raw_path must be relative to the project root")
    resolved = (project_root / relative_path).resolve()
    try:
        resolved.relative_to(project_root.resolve())
    except ValueError as exc:
        raise ValueError("R11 audit raw_path escapes the project root") from exc
    return resolved
